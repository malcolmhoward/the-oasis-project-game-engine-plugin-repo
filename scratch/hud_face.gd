...
enum Expression { NEUTRAL, HAPPY, WORRIED, CURIOUS, SLEEPING }

var _current_expression: Expression = Expression.NEUTRAL
...
func _process(delta: float):
	...
	_idle_timer += delta
	if _idle_timer > 30.0 and _current_expression == Expression.NEUTRAL:
		_set_expression(Expression.SLEEPING)

	# Expression timeout — return to neutral
	if _current_expression != Expression.NEUTRAL and _current_expression != Expression.SLEEPING:
		_expression_timer += delta
		if _expression_timer >= _expression_duration:
			_set_expression(Expression.NEUTRAL)
...
func _on_mqtt_message(_topic: String, _payload: String):
	...
	# Wake up if sleeping
	if _current_expression == Expression.SLEEPING:
		_set_expression(Expression.NEUTRAL)
	...
	if msg is Dictionary:
		...
		if msg.has("temp") and float(msg.get("temp", 22)) > 35:
			_set_expression(Expression.WORRIED)
		...
		if msg.get("status") == "online" and _current_expression == Expression.NEUTRAL:
			_set_expression(Expression.HAPPY)
...
func _on_peer_discovered(_peer_id: String, _data: Dictionary):
	_set_expression(Expression.CURIOUS)
...
func _set_expression(expr: Expression):
...
func _draw_face(canvas: Control):
	...
	if _is_blinking or _current_expression == Expression.SLEEPING:
		...
		if _current_expression == Expression.WORRIED:
			eye_color = ArcReactorDark.STATUS_WARNING
		elif _current_expression == Expression.CURIOUS:
